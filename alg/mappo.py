...
from alg.vqvae import SequentialVQVAE
from alg.vae import SequentialVAE
from alg.suptom import SupTomActor, SupTom_RNNActor
import os
...
class MPE_MAPPO:
    ...
    def __init__(
        ...
        self.gamma = config.alg.gamma
        self.gaelambda = config.alg.gaelambda
        self.ppo_epochs = config.alg.ppo_epochs
        self.eps_clip = config.alg.eps_clip
        self.action_dim = config.env.action_dim
        self.use_rnn = config.training.use_rnn
        self.minibatch_size = config.training.minibatch_size
        self.normalize_value = config.training.normalize_value
        self.value_clipping = config.training.value_clipping
        ...
        if self.use_rnn:
            ...
            self.actor = MPE_Actor(
                action_dim=config.env.action_dim, obs_dim=config.env.obs_dim
            ).to(self.device)
            self.critic = MPE_Critic(state_dim=config.env.state_dim).to(self.device)
            self.ac_parameters = list(self.actor.parameters()) + list(
                self.critic.parameters()
            )
        self.optimizer = torch.optim.Adam(
            self.ac_parameters, lr=config.alg.lr, eps=1e-5
        )
...
class MPE_GFN_MAPPO:
    ...
    def __init__(
        ...
        self.gfn_dict_size = config.module.gfn_dict_size
        self.gfn_sampling_exponent = config.module.gfn_sampling_exponent
        self.gfn_num_samples = config.module.gfn_num_samples
    ...
    def select_action(
        ...
        with torch.no_grad():
            ...
            if self.use_rnn:
                logits, actor_memory = self.actor(
                    obs, actor_memory, in_latents.float().to(self.device)
                )
                value, critic_memory = self.critic(
                    obs.flatten(1).unsqueeze(1).repeat(1, self.n_agents, 1),
                    critic_memory,
                    in_latents.unsqueeze(1)
                ...
                logits = self.actor(obs, in_latents.float().to(self.device))
                value = self.critic(
                    obs.flatten(1).unsqueeze(1).repeat(1, self.n_agents, 1),
                    in_latents.unsqueeze(1)
                    .repeat_interleave(self.n_agents, 1)
            ...
            for index in torch.utils.data.sampler.BatchSampler(
                ...
                if self.use_rnn:
                    ...
                    logits_now = self.actor(
                        old_states[index].float(), in_latents[index].float()
                    )
                    values_now = self.critic(
                        old_states[index]
                        .flatten(2)
...
class MPE_VQVAE_MAPPO:
    ...
    def __init__(
        ...
        self.vqvae = SequentialVQVAE(config, self.device)
        self.vqvae_dict_size = config.module.vqvae_dict_size
        self.vqvae_state_size = config.module.vqvae_state_size
    ...
    def select_action(
        ...
        with torch.no_grad():
            obs = torch.from_numpy(np.stack(obs, axis=0)).float().to(self.device)
            _, latents, next_vqvae_memory = self.vqvae.sample_latents(
                obs,
                prev_actions,
                rewards,
                vqvae_memory,
            ...
            in_latents = (
                torch.nn.functional.one_hot(latents.long(), self.vqvae_dict_size)
                .float()
                .flatten(-2)
            )
            if self.use_rnn:
                ...
                return action, logprobs, value.squeeze(), latents, next_vqvae_memory
    ...
    def save_model(self, model_dir, step):
        ...
        save_path = os.path.join(model_dir, "model_vqvae.pth")
        torch.save(save_dict, save_path)
    ...
    def load_model(self, saved_dict):
        ...
        self.actor.load_state_dict(saved_dict["actor"])
        self.critic.load_state_dict(saved_dict["critic"])
        self.vqvae.load_state_dict(saved_dict["vqvae"])
...
class MPE_VAE_MAPPO:
    ...
    def __init__(
        ...
        self.vae = SequentialVAE(config, self.device)
        self.vae_latent_size = config.module.vae_latent_size
    ...
    def select_action(
        ...
        with torch.no_grad():
            obs = torch.from_numpy(np.stack(obs, axis=0)).float().to(self.device)
            latents, next_vae_memory = self.vae.sample_latents(
                obs,
                prev_actions,
                rewards,
                vae_memory,
            )
            in_latents = latents.float()
            if self.use_rnn:
                logits, actor_memory = self.actor(
                    obs, actor_memory, in_latents.float().to(self.device)
                )
                ...
                return action, logprobs, value.squeeze(), latents, next_vae_memory
    ...
    def save_model(self, model_dir, step):
        ...
        save_path = os.path.join(model_dir, "model_vae.pth")
        torch.save(save_dict, save_path)
    ...
    def load_model(self, saved_dict):
        ...
        self.actor.load_state_dict(saved_dict["actor"])
        self.critic.load_state_dict(saved_dict["critic"])
        self.vae.load_state_dict(saved_dict["vae"])


class MPE_SUPTOM_MAPPO:
    ...
    def __init__(
        ...
        if self.use_rnn:
            self.actor = SupTom_RNNActor(config).to(self.device)
            self.critic = MPE_RNN_Critic(config).to(self.device)

            goal_params = list(self.actor.belief_net.parameters())
            goal_param_ids = {id(p) for p in goal_params}
            actor_params = [
                p for p in self.actor.parameters() if id(p) not in goal_param_ids
            ]
            self.ac_parameters = actor_params + list(self.critic.parameters())
        else:
            self.actor = SupTomActor(config).to(self.device)
            self.critic = MPE_Critic(config).to(self.device)
    ...
    def select_action(
        ...
        with torch.no_grad():
            obs = torch.from_numpy(np.stack(obs, axis=0)).float().to(self.device)
            if self.use_rnn:
                logits, actor_memory, beliefs, others_beliefs = self.actor(
                    obs, actor_memory
                )
                value, critic_memory = self.critic(
                    obs.flatten(1).unsqueeze(1).repeat(1, self.n_agents, 1),
                ...
                logits, beliefs, others_beliefs = self.actor(obs)
                value = self.critic(
                    obs.flatten(1).unsqueeze(1).repeat(1, self.n_agents, 1),
                    beliefs.unsqueeze(1)
                    .repeat_interleave(self.n_agents - 1, 1)
                    .unsqueeze(1)
                    .repeat_interleave(self.n_agents, 1)
                    .unsqueeze(-2)
                    .float()
                ...
                return action, logprobs, value.squeeze(), beliefs, others_beliefs
        ...
        for _ in range(self.ppo_epochs):
            ...
            for index in torch.utils.data.sampler.BatchSampler(
                ...
                if self.use_rnn:
                    ...
                    logits_now, beliefs, others_beliefs = self.actor(
                        old_states[index].float()
                    )
                    values_now = self.critic(
                        old_states[index]
                        .flatten(2)
                        ...
                        .float(),
                        beliefs.unsqueeze(2)
                        .repeat_interleave(self.n_agents - 1, 2)
                        .unsqueeze(2)
                        .repeat_interleave(self.n_agents, 2)
                        .unsqueeze(-2)
                        .float(),

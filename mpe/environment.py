...
class MultiAgentEnv(gym.Env):
    ...
    #             # self.viewers[i] = rendering.SimpleImageViewer()
    ...
    def render(self, mode="rgb_array", size=700):
        assert mode == "rgb_array", "Headless renderer only supports rgb_array"
        import cv2
        ...
        # camera center
        if self.shared_viewer:
            cam_pos = np.zeros(self.world.dim_p)
        else:
            cam_pos = self.agents[0].state.p_pos

        # world → pixel transform
        def world_to_px(pos):
            x = (pos[0] - (cam_pos[0] - cam_range)) / (2 * cam_range)
            y = (pos[1] - (cam_pos[1] - cam_range)) / (2 * cam_range)
            px = int(x * size)
            py = int((1 - y) * size)  # flip y-axis
            return px, py

        # --- draw entities ---
        for entity in self.world.entities:
            px, py = world_to_px(entity.state.p_pos)

            radius = int(entity.size / (2 * cam_range) * size)

            color = (np.array(entity.color) * 255).astype(np.uint8)
            color = tuple(int(c) for c in color[::-1])  # RGB → BGR for cv2

            if "agent" in entity.name:
                # semi-transparent → approximate with lighter color
                color = tuple(int(0.5 * c + 127) for c in color)

            cv2.circle(img, (px, py), radius, color, -1)

            # --- communication circles ---
            if hasattr(entity, "silent") and not entity.silent:
                dim_c = self.world.dim_c
                for ci in range(dim_c):
                    offset = (ci * 2 - dim_c + 1) * radius // dim_c
                    cx = px + offset
                    cy = py

                    val = entity.state.c[ci] if hasattr(entity.state, "c") else 0
                    comm_color = int((1 - val) * 255)
                    comm_color = (comm_color, comm_color, comm_color)

                    cv2.circle(img, (cx, cy), radius // dim_c, comm_color, -1)
        ...
        for wall in self.world.walls:
            ...
            pts = np.array([world_to_px(p) for p in corners], dtype=np.int32)

            color = (np.array(wall.color) * 255).astype(np.uint8)

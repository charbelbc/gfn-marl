    ...
    def render(
        ...
        for j in range(0, self.height):
            for i in range(0, self.width):
                ...
                #######
                # dist = abs(i - agents[0].pos[0]) + abs(j - agents[0].pos[1])
                # alpha = np.exp(-dist / 3.5)
                # c_far = np.array([48, 60, 84], dtype=np.float32)
                # c_near = np.array([230, 228, 66], dtype=np.float32)
                # c = alpha * c_near + (1 - alpha) * c_far
                # tile_img = (
                #     tile_img.astype(np.float32) * alpha + c * (1 - alpha)
                # ).astype(np.uint8)
                #######

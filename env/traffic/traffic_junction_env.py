...
from env.traffic.traffic_helper import *
...
class TrafficJunctionEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        self.dim = config.env.dim
        self.vision = config.env.vision
        self.ncar = config.env.num_agents
        self.add_rate_min = config.env.add_rate_min
        self.add_rate_max = config.env.add_rate_max
        self.curr_start = config.env.curr_start
        self.curr_end = config.env.curr_end
        self.difficulty = config.env.difficulty
        self.vocab_type = config.env.vocab_type
    ...
    def _initialize(self):
        ...
        self.ncar = self.config.env.num_agents
        self.dims = dims = (self.dim, self.dim)
        difficulty = self.config.env.difficulty
        vision = self.config.env.vision
    ...
    def render(self, mode="rgb_array", cell_size=32):
        """
        Returns an RGB frame of shape
        (H*cell_size, W*cell_size, 3)
        """

        h, w = self.dims
        frame = np.ones((h * cell_size, w * cell_size, 3), dtype=np.uint8) * 255

        # Colors
        COLOR_BG = np.array([255, 255, 255])
        COLOR_ROAD = np.array([180, 180, 180])
        COLOR_GAS = np.array([220, 30, 30])
        COLOR_BRAKE = np.array([30, 30, 220])
        COLOR_CRASH = np.array([255, 200, 0])

        # Draw roads
        for i in range(h):
            for j in range(w):
                if self.grid[i, j] != self.OUTSIDE_CLASS:
                    frame[
        ...
        # Count cars per cell
        occupancy = {}
        for pos in self.car_loc:
            pos = tuple(pos)
            if pos != (0, 0):
                occupancy[pos] = occupancy.get(pos, 0) + 1

        # Draw cars
        for idx, pos in enumerate(self.car_loc):
    ...
    def _set_paths_medium_old(self):
        h, w = self.dims
        self.routes = {"TOP": [], "LEFT": [], "RIGHT": [], "DOWN": []}
